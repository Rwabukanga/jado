...
function myum_url() {
	  var myum_url = "http://localhost:4003";
	...
	var form_data = $("#l_form").serializeToJSON({
		...
		$.ajax({ 
            ...
            success : function(result, textStatus, jQxhr) {
            	...
            	if(result.CODE==200){
            		userid=result.OBJECT.user.objectId;
...
function lock_user(username){
	...
	$.ajax({ 
        url: myum_url()+'/users/lock/'+username, 
        type : "POST", // type of action POST || GET
        dataType : 'json', // data type
...
function login_user_in(user, permission){
	...
	$.ajax({ 
        ...
        type : "POST", // type of action POST || GET
        //dataType : 'json', // data type
        data : {username:user.username,fname:user.firstname,lname:user.lastname,role:user.role},// post data || get data
       // contentType: 'application/json',
        asynch:false,
        success : function(result, textStatus, jQxhr) {
